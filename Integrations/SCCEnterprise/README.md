...
|Verify SSL|If enabled, verify the SSL certificate for the connection to the ServiceNow is valid.||Boolean|true|
|Table Role|Specify the role that should be set to access the newly created table. If nothing is provided, action will create a new role "u_scc_enterprise_cloud_posture_ticket_user".||String|u_scc_enterprise_cloud_posture_ticket_user|
...
|Verify SSL|If enabled, verify the SSL certificate for the connection to Jira is valid.||Boolean|true|
...
|Location ID||String|global|
|PubSub Project ID||String||
|Quota Project ID||String||
|User Service Account||Password|*****|
|Workload Identity Email||String||
...
|Location ID|ID of the location that should be used in Google Security Command Center integration. Defaults to ‘global’||String|global|
|PubSub Project ID|ID of the project that will be used in the integration to host the Pub/Sub Topic and Subscription connection.||String||
...
|User Service Account|Service Account that is used for authentication. If both this and "Workload Identity Email" not provided, the default Service Account of the SecOps Instance will be used to authenticate.||Password|*****|
|Workload Identity Email|A Service Account Client Email to replace the usage of "User's Service Account", which will be used for Impersonation. Note that the SOAR Service Account must be granted the "Service Account Token Creator" IAM role on the User Service Account. If both this and "User's Service Account" not provided, the default Service Account of the SecOps Instance will be used to authenticate.||String||
|Owner Tag Name|Name of the tag that contains information about the owner of the finding.||String|Owner Name|
|Fallback Owner|Email address of the owner of the alerts created by this connector.|True|String||
|Group By GCP Project|If enabled, the connector will group findings by project.||Boolean|true|
|Group By AWS Account|If enabled, the connector will group findings by AWS account.||Boolean|true|
|Group By Azure Subscription|If enabled, the connector will group findings by Azure subscription.||Boolean|true|
|Group By Severity|If enabled, the connector will group findings by severity.||Boolean|true|
|Group By Asset Type|If enabled, the connector will group findings by asset type.||Boolean|true|
|Verify SSL|If enabled, verify the SSL certificate for the connection to the Google Security Command Center server is valid.||Boolean|true|
|Disable Overflow|If enabled, the connector will disable the overflow mechanism.||Boolean|true|
