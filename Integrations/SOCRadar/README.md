...
|Max IOCs Per Feed|False|Integer|5000|
...
|EventClassId|The field name used to determine the event name (sub-type).|True|String|alarm_id|
...
|PythonProcessTimeout|Timeout limit (in seconds) for the Python process running the connector.|True|Integer|180|
|API Root|SOCRadar API base URL.|True|String|https://platform.socradar.com/api|
|API Key|SOCRadar platform API key with Incident API V4 permissions.|True|Password|*****|
|Company ID|SOCRadar company identifier (numeric).|True|String||
|Verify SSL|Enable SSL certificate verification for API requests.|False|Boolean|true|
|Max Alerts Per Cycle|Maximum number of alarms to ingest per polling cycle.|False|Integer|100|
