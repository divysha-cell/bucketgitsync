...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Search Mode|Specify the mode for search execution.||List|Smart|
...
|Results count limit|Specify how many results to return. Note: this parameter appends the “head” key word to the provided query. Default is 100.||String|100|
|Results From|Specify the start time for the query. Default: -24h||String|-24h|
|Results To|Specify the end time for the query. Default: now.||String|now|
|Result fields|Specify a comma-separated list of fields that need to be returned. Note: this parameter appends "fields" key word to the provided query.||String||
...
|Result fields|Specify a comma-separated list of fields that need to be returned.||String||
|Index|Specify what index should be used, when searching for events related to the host. If nothing is provided, action will not use index.||String||
|Host Key|Specify what key should be used to get information about host events. Default: host.||String|host|
...
|Status|Specify the new status for notable events.||List|Select One|
|Urgency|Specify the new urgency for the notable event.||List|Select One|
|New Owner|Specify the new owner of the notable event.||String||
|Comment|Specify comment for the notable event.||String||
|Disposition|Specify the disposition for the notable event.||List|Select One|
...
|Results count limit|Specify how many results to return. Note: this parameter appends the "head" key word to the provided query. Default is 100.||String||
|Results from|Specify the start time for the query. Default: -24h||String|-24h|
|Results To|Specify the end time for the query. Default: now.||String|now|
|Result fields|Specify a comma-separated list of fields that need to be returned. Note: this parameter appends "fields" key word to the provided query.||String||
...
|Hostname Entity Key|Specify what key should be used with Hostname entities, when preparing the. Please refer to the action documentation for details.||String||
...
|Email Address Entity Key|Specify what key should be used with Email Address entities. Please refer to the action documentation for details.||String||
...
|Host|Specify the host that is related to the event.||String||
|Source|Specify the source of the event. Example: www.||String||
|Sourcetype|Specify the source type of the event. Example: web_event||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Username|Username of the Splunk account.||String||
...
|Rule Generator Field Name|The name of the field that will be used to map the rule generator value. Note: only information about notable event itself is used for mapping, events are disregarded. If invalid value is provided, connector will use "rule_name" as field.||String||
|Server Address|Server address of the Splunk instance.|True|String|https://<ip>:8089|
|Password|Password of the Splunk account.||Password|*****|
|API Token|Splunk API token. API token has priority over other authentication methods, when this field is not empty.||Password|*****|
...
|Padding Time|Amount of hours that will be used as a padding. If nothing is provided, this parameter is not going to be applied. Max 100 hours. Note: it's not safe to increase the padding period beyond 12 hours.||Integer||
|Only Drilldown Events|If enabled, the connector will only try to fetch drilldown events, without trying to fetch base events. Note: this parameter requires 'Extract Base Events' to be enabled.||Boolean|false|
|Max Notable Events To Fetch|How many notable events to process per one connector iteration.||Integer|50|
|Use whitelist as a blacklist|If enabled, whitelist will be used as a blacklist.||Boolean|false|
|Verify SSL|If enabled, verify the SSL certificate for the connection to the Splunk server is valid.||Boolean|false|
...
|Query Filter|Additional filter for the query that is sent to Splunk to get notable events. Value provided here will be appended to the WHERE clause of the query. Please refer to documentation portal for more information.||String||
|CA Certificate File|CA Certificate File||String||
|Extract Base Events|If enabled, connector will try to extract base events related to notable event using information about the job. In other case, connector will create a Siemplify Event based on the notable event. Note: that if parameter is set to True, but connector can't work with jobs, connector will use information about notable event as a fallback mechanism.||Boolean|true|
|Notable Event Data Along Base Event|If enabled, connector will create Siemplify Events based on both Notable Event and Base Events together.||Boolean|false|
|Multivalue Fields|A comma-separated list of fields that contain multiple entities. If, for example, one field can contain two hostnames, we would need to split notable event into two Siemplify Events in order to do a correct mapping of entities.||String|asset, src, dest, ip|
|Alert Name Source|Source for the alert name. Possible values: Search Name, Rule Name.||String|Search Name|
...
|Username|Splunk account username.||String||
|Password|Splunk account password.||Password|*****|
|API Token|Splunk API Token. API token has priority over other authentication methods, when this field is not empty.||Password|*****|
|Time Frame|Time frame of alerts to fetch. Examples: 1m - from 1 minute ago 3h - from 3 hours ago 1d - from 24 hours ago (1 day) 1w - from 1 week ago.||String|1h|
|Alerts Count Limit|Limit the number of Alerts returned by the connector per 1 iteration.||Integer|100|
|Verify SSL|Whether to verify ssl certificate on connection or not.||Boolean|true|
...
|EventClassId|The field name used to determine the event name (sub-type),  e.g: event_type_field_name||String|app|
...
|Environment Field Name|Field which represent the environment in the event data.||String||
...
|Events Count Limit Per Query|Max amount of events to fetch per query, e.g: 10||String|10|
|Max Days Backwards|Number of days before the first connector iteration to retrieve events from. This parameter applies to the initial connector iteration after you enable the connector for the first time, or used as a fallback value in cases where connector's last run timestamp expires.||String|1|
|Aggregate Events Query|If enabled, the connector will combine all events under one alert.||Boolean|FALSE|
|Proxy Server Address|The address of the proxy server to use.||String||
|Proxy Username|The proxy username to authenticate with.||String||
|Proxy Password|The proxy password to authenticate with.||Password|*****|
|CA Certificate File|CA Certificate File||String||
