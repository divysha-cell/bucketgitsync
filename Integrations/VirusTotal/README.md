...
|Linux Server Address|Linux server address(e.g: x.x.x.x).||String||
|Linux User|Linux User||String||
|Linux Password|Linux Password||Password|*****|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Threshold|Specify the accepted threshold for the detected samples related to the IP address. If the number of engines that marked related samples as malicious is higher than the specified threshold, IP address will be marked as suspicious.||String|25|
...
|Rescan after days|Action will fetch the latest result. If the result is older than mentioned days it will automatically rescan the entity||String||
