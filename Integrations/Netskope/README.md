...
|V1 Api Key|None||Password|*****|
|V2 Api Key|None||Password|*****|
|Client ID|None||String|None|
...
|Entries|A comma-separated list of URLs, domains, or IP addresses to add to the list. The action also automatically processes URL, Domain, and IP Address entities attached to the case.||String||
|Deploy URL List Changes|If selected, the action deploys all pending changes to all URL lists. Note: All changes across all URL lists are deployed.||Boolean||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Query|This acts as a filter for all the cloud app events in the alerts database.||String||
|Type|The type of the alert to filter by. Valid values: anomaly | 'Compromised Credential' | policy | 'Legal Hold' | malsite | Malware | DLP | watchlist | quarantine | Remediation.||String||
|Time Period|Time period to search alerts at (milliseconds backwards). Valid Values: 3600 | 86400 | 604800 | 2592000.||String||
|Start Time|Restrict alerts to those that have timestamps greater than this (unixtime). Needed only if time period is not passed.||String||
|End Time|Restrict alerts to those that have timestamps less than this (unixtime). Needed only if time period is not passed.||String||
|Is Acknowledged|Whether to get only acknowledged alerts.||Boolean||
|Limit|Number of results to return. Default: 100.||String||
...
|Quarantine Profile ID|ID of a quarantine profile. This parameter is mandatory for V1 API.||String||
|Use V2 API|If enabled, the action will use the V2 API endpoint (requires credentials).||Boolean||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Query|This acts as a filter on all the entries in the database.||String||
|Limit|Number of results to return. Default: 25.||String||
|Use V2 API|If enabled, the action will use the V2 API endpoint (requires credentials).||Boolean||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Start Time|Restrict events to those that have timestamps greater than this (unixtime). Needed only if time period is not passed.||String||
|End Time|Restrict events to those that have timestamps less than this (unixtime). Needed only if time period is not passed.||String||
|Max Items To Return|Maximum number of items to retrieve. If unspecified, the maximum (100) is used.||String||
|Use V2 API|If enabled, the action will use the V2 API endpoint (requires credentials).||Boolean||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Query|This acts as a filter for all the cloud app events in the events database.||String||
|Type|The type of the alert to filter by. Valid values: page | application | audit | infrastructure.||String||
|Time Period|Time period to search events at (milliseconds backwards). Valid Values: 3600 | 86400 | 604800 | 2592000.||String||
|Start Time|Restrict events to those that have timestamps greater than this (unixtime). Needed only if time period is not passed.||String||
|End Time|Restrict events to those that have timestamps less than this (unixtime). Needed only if time period is not passed.||String||
|Limit|Number of results to return. Default: 100.||String||
