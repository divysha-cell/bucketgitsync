...
|----|-----------|-----------|----|------------|
|API Root|None|True|String|https://yourdomain.freshservice.com|
...
|Rows per Page|Specify how many ticket time entries should be returned per page for Freshservice pagination.||String|30|
|Start at Page|Specify starting from which page ticket time entries should be returned with Freshservice pagination.||String|1|
|Max Rows to Return|Specify how many ticket time entries action should return in total.||String|30|
...
|Agent Email|Specify agent email for whom to change a ticket time entry.||String||
|Note|Specify a note for the ticket time entry.||String||
|Time Spent|Specify time spent for ticket time entry. Format: {hh:mm}||String||
|Billable|If enabled, ticket time entry will be marked as billable.||Boolean|false|
|Custom Fields|Specify a JSON object that contains custom fields to add to the ticket time entry. Action appends new custom fields to any existing for ticket time entry. Example format: {"key1":"value1","key2":"value2"}||String||
...
|Note|Specify a note to add to the ticket time entry.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Requester Email|Specify the email address to return requester records for.||String||
|Rows per Page|Specify how many requester records should be returned per page for Freshservice pagination.||String|30|
|Start at Page|Specify starting from which page requester records should be returned with Freshservice pagination.||String|1|
|Max Rows to Return|Specify how many requester records action should return in total.||String|30|
...
|Rows per Page|Specify how many ticket conversations should be returned per page for Freshservice pagination.||String|30|
|Start at Page|Specify starting from which page ticket conversations should be returned with Freshservice pagination.||String|1|
|Max Rows to Return|Specify how many ticket conversations action should return in total.||String|30|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Agent Email|Specify the email address to return agent records for.||String||
|Agent State|Specify agent states to return.||List|All|
|Include not Active Agents|If enabled, results will include not active agent records.||Boolean|false|
|Rows per Page|Specify how many agent records should be returned per page for Freshservice pagination.||String|30|
|Start at Page|Specify starting from which page agent records should be returned with Freshservice pagination||String|1|
|Max Rows to Return|Specify how many agent records action should return in total.||String|30|
...
|Last Name|Specify the last name of the agent to create.||String||
|Is occasional|If enabled, agent will be created as an occasional agent, otherwise full-time agent will be created.||Boolean|false|
|Can See All Tickets From Associated Departments|If enabled, agent will be able to see all tickets from Associated departments.||Boolean|false|
|Departments|Specify department names associated with the agent. Parameter accepts multiple values as a comma separated string.||String||
|Location|Specify location name associated with the agent.||String||
|Group Memberships|Specify group names agent should be a member of.||String||
...
|Job Title|Specify agent job title.||String||
|Custom Fields|Specify a JSON object that contains custom fields to add to the agent. Action appends new custom fields to any existing for an agent. Example format: {"key1":"value1","key2":"value2"}||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Job Title|Specify requester job title.||String||
...
|Last Name|Specify the last name of the requester to create.||String||
|Can See All Tickets From Associated Departments|If enabled, requester will be able to see all tickets from associated departments.||Boolean|false|
|Departments|Specify department names associated with the requester. Parameter accepts multiple values as a comma separated string.||String||
|Location|Specify location name associated with the requester.||String||
|Custom Fields|Specify a JSON object that contains custom fields to add to the requester. Action appends new custom fields to any existing for requester. Example format: {"key1":"value1","key2":"value2"}||String||
...
|Status|Specify new status for the ticket.||List|Not Changed|
|Subject|Specify subject field to update.||String||
|Description|Specify description field to update.||String||
|Requester Email|Specify requester email to update.||String||
|Agent Assign To|Specify the agent email to update.||String||
|Group Assign To|Specify the group name to update.||String||
|Priority|Specify priority to update.||List|Not Changed|
|Urgency|Specify urgency to update.||List|Not Changed|
|Impact|Specify impact to update.||List|Not Changed|
|Tags|Specify tags to replace in the ticket. Parameter accepts multiple values as a comma separated string. Note that due to the Freshservice API limitations action replaces existing tags in the ticket, not appending new ones to existing.||String||
|Custom Fields|Specify a JSON object that contains custom fields to add to the ticket. Action appends new custom fields to any existing for a ticket. Example format: {"key1":"value1","key2":"value2"}||String||
|File Attachments to Add|Specify the full path for the file to be uploaded with the ticket. Parameter accepts multiple values as a comma separated string. The total size of the attachments must not exceed 15MB||String||
...
|Agent Assign To|Specify the agent email to assign the ticket to.||String||
|Group Assign To|Specify the group name to assign the ticket to.||String||
|Department Name|Specify the name of the department for the ticket. This parameter is case sensitive.||String||
|Priority|Specify priority to assign to the ticket.|True|List|Medium|
|Urgency|Specify urgency to assign to the ticket.||List|Medium|
|Impact|Specify impact to assign to the ticket.||List|Medium|
|Tags|Specify tags to assign to the ticket. Parameter accepts multiple values as a comma separated string.||String||
|Custom Fields|Specify a JSON object that contains custom fields to add to the ticket. Action appends new custom fields to any existing for a ticket. Example format: {"key1":"value1","key2":"value2"}||String||
|File Attachments to Add|Specify the full path for the file to be uploaded with the ticket. Parameter accepts multiple values as a comma separated string. The total size of the attachments must not exceed 15MB||String||
...
|----|-----------|-----------|----|------------|
|Ticket ID|Specify ticket ID to return conversations for.|True|String||
|Note Type|Specify the type of note action should add to the ticket.||List|Private|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Ticket Type|Specify ticket type to return.||List|All|
|Requester|Specify the requester email of tickets to return.||String||
|Include Stats|If enabled, action will return additional stats on the tickets.||Boolean|False|
|Search for Last X hours|Specify the timeframe to search tickets for.||String||
|Rows per Page|Specify how many tickets should be returned per page for Freshservice pagination.||String|30|
|Start at Page|Specify starting from which page tickets should be returned with Freshservice pagination.||String|1|
|Max Rows to Return|Specify how many tickets action should return in total.||String|30|
|Workspace ID|Specify the ID of the workspace that should be used to list tickets. If nothing is provided, the action will list tickets only from the primary workspace. To list tickets from all workspaces provide “0” in the parameter.||String||
...
|Email|Specify the email of the requester to update.||String||
|First Name|Specify the first name of the requester to update.||String||
|Last Name|Specify the last name of the requester to update.||String||
|Can See All Tickets From Associated Departments|If enabled, requester will be able to see all tickets from associated departments.||Boolean|false|
|Departments|Specify department names associated with the requester. Parameter accepts multiple values as a comma separated string.||String||
|Location|Specify location name associated with the requester.||String||
|Job Title|Specify requester job title.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Last Name|Specify the last name of the agent to update.||String||
|Is occasional|If enabled, agent will be updated as an occasional agent, otherwise it will be a full-time agent.||Boolean|false|
|Can See All Tickets From Associated Departments|If enabled, agent will be able to see all tickets from Associated departments.||Boolean|false|
|Departments|Specify department names associated with the agent. Parameter accepts multiple values as a comma separated string.||String||
|Location|Specify location name associated with the agent.||String||
|Group Memberships|Specify group names agent should be a member of.||String||
|Roles|Specify roles to add to agent. Parameter accepts multiple values as a comma separated string. Example: {'role_id':170000XXXXX,'assignment_scope': 'entire_helpdesk'}||String||
|Job Title|Specify agent job title.||String||
|Custom Fields|Specify a JSON object that contains custom fields to add to the agent. Action appends new custom fields to any existing for an agent. Example format: {"key1":"value1","key2":"value2"}||String||
|First Name|Specify the first name of the agent to update.||String||
|Agent ID|Specify agent id to update.|True|String||
|Email|Specify the email of the agent to update.||String||
...
|Fetch Private Notes?||Boolean|false|
...
|Verify SSL|If enabled, integration will try to verify that API Root is configured with a valid certificate.||Boolean|true|
...
|Minimum Priority to Fetch|Minimum priority of the ticket to be ingested to Siemplify, for example, Low or Medium. Possible values: Low, Medium, High, Urgent||String|Medium|
|Tickets Status to Fetch|Ticket statuses to be ingested to Siemplify. Parameter accepts multiple values as a comma separated string. Possible values: Open, Pending, Resolved, Closed||String|Open, Closed|
...
|Workspace ID|ID of the workspace that should be used to fetch tickets. If nothing is provided, the action will fetch tickets only from the primary workspace. To fetch tickets from all workspaces provide “0” in the parameter.||String||
