<p align="center"><img src="./Resources/ConnectWise.svg" 
     alt="ConnectWise" width="200"/></p>
...
|Custom Close Status|If the specific system use a custom closed status (e.g. Completed)||String||
...
|Internal|If checked, put comment in internal section||Boolean||
...
|Owner Name|ConnectWise member name to assign this ticket to, e.g. connectwise_user_1.||String||
...
|Email Note CC|Specify a comma-separated list of email addresses that should receive all of the notes via email.||String||
...
|Summary|Specify the summary for the updated ticket. Note: if the summary is more than 100 characters, it will be truncated.||Content||
|Type Name|e.g. Application||String||
|SubType Name|e.g. Adobe||String||
|Item Name|e.g. Development||String||
|Owner Name|ConnectWise member name to assign this ticket to, e.g. connectwise_user_1.||String||
|Board|Board name.||String||
|Priority|e.g. Priority 3 - Normal Response.||String||
|Status|New ticket status, e.g. In Progress (plan of action)||String||
|Email Note CC|Specify a comma-separated list of email addresses that should receive all of the notes via email.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Allow Only Owner Update|If enabled, action will only allow the owner to update the attachment.||Boolean|true|
|Display In Customer Portal|If enabled, attachment will be shown in the customer portal.||Boolean|true|
