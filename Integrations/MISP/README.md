...
|Use SSL|None||Boolean|False|
|CA Certificate File - parsed into Base64 String|None||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers to which you want to add a new sighting. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to search for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“.||String||
|Category|Specify a comma-separated list of categories. If specified, action will only add sightings to attributes that have matching category. If nothing is specified, action will ignore categories in attributes. Possible values: External Analysis, Payload Delivery, Artifacts Dropped, Payload Installation.||String||
|Type|Specify a comma-separated list of attribute types. If specified, action will only add sightings to attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Sightings Type|Specify the type of the Sighting.|True|List|Sighting|
|Source|Specify the source for the sighting. Example: SIEM, SOAR, Siemplify.||String||
|Date Time|Specify the date time for the sighting. Format: 2020-02-10 11:00:00.||String||
|Object UUID|Specify the uuid of the object that contains the desired attribute||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and add sighting for all attributes that match our criteria. ||List|Provided Event|
|Attribute UUID|Specify a comma-separated list of attribute UUIDs to which you want to add a new sighting. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values. ||String||
...
|Create Insights|If enabled, action will generate an insight for every entity that was fully processed.||Boolean|true|
|Threat Level Threshold|Specify what should be the threshold for the threat level of the event, where the entity was found. If related event exceeds or matches threshold, entity will be marked as suspicious.||List|Low|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Events Limit|Specify max amount of events to fetch. If not specified, all events will be fetched.||String||
|Mark As Suspicious|If enabled, action will mark entity as suspicious, if there is at least one related event to it.||Boolean|true|
...
|FILENAME|Specify the name of the file, which you want to add to the event.||String||
|MD5|Specify the md5 of the file, which you want to add to the event.||String||
|SHA1|Specify the sha1 of the file, which you want to add to the event.||String||
|SHA256|Specify the sha256 of the file, which you want to add to the event.||String||
|SSDEEP|Specify the ssdeep of the file, which you want to add to the event. Format: size:hash:hash||String||
|Use Entities|If enabled, action will use entities in order to create objects. Supported entities: File name and hash. “Use Entities“ has priority over other parameters.||Boolean|false|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers for which you want to list sightings. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to seach for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“.||String||
...
|Type|Specify a comma-separated list of attribute types. If specified, action will only list sightings for attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and list sightings for all attributes that match our criteria.||List|Provided Event|
|Attribute UUID|Specify a comma-separated list of attribute UUIDs for which you want to list sightings. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
...
|Dst-port|Specify the destination port, which you want to add to the event.||String||
|Src-port|Specify the source port, which you want to add to the event.||String||
|Hostname-src|Specify the source hostname, which you want to add to the event.||String||
|Hostname-dst|Specify the source destination, which you want to add to the event.||String||
|IP-Src|Specify the source IP, which you want to add to the event.||String||
|IP-Dst|Specify the destination IP, which you want to add to the event.||String||
|Layer3-protocol|Specify the related layer 3 protocol, which you want to add to the event.||String||
|Layer4-protocol|Specify the related layer 4 protocol, which you want to add to the event.||String||
|Layer7-protocol|Specify the related layer 7 protocol, which you want to add to the event.||String||
|Use Entities|If enabled, action will use entities in order to create objects. Supported entities: IP Address. “Use Entities“ has priority over other parameters.||Boolean|false|
|IP Type|Specify what attribute type should be used with IP entities.||List|Source IP|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers that you want to delete. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to search for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“ or Object UUID is provided.||String||
|Category|Specify a comma-separated list of categories. If specified, action will only delete attributes that have matching category. If nothing is specified, action will ignore categories in attributes. Possible values: External Analysis, Payload Delivery, Artifacts Dropped, Payload Installation.||String||
|Type|Specify a comma-separated list of attribute types. If specified, action will only delete attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Object UUID|Specify the uuid of the object that contains the desired attribute||String||
|Attribute UUID|Specify a comma-separated list of attribute UUIDs that you want to delete. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and delete all attributes that match our criteria.||List|Provided Event|
...
|URL|Specify the URL, which you want to add to the event.||String||
|Port|Specify the port, which you want to add to the event.||String||
|First seen|Specify, when the URL was first seen. Format: 2020-12-22T13:07:32Z||String||
|Last seen|Specify, when the URL was last seen. Format: 2020-12-22T13:07:32Z||String||
|Domain|Specify the domain, which you want to add to the event.||String||
|Text|Specify the additional text, which you want to add to the event.||String||
|IP|Specify the IP, which you want to add to the event.||String||
|Host|Specify the Host, which you want to add to the event.||String||
|Use Entities|If enabled, action will use entities in order to create objects. Supported entities: URL. “Use Entities“ has priority over other parameters.||Boolean|false|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers for which you want to set an IDS flag. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to seach for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“.||String||
...
|Type|Specify a comma-separated list of attribute types. If specified, action will only set IDS flag for attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and set IDS flag for all attributes that match our criteria.||List|Provided Event|
|Attribute UUID|Specify a comma-separated list of attribute UUIDs for which you want to set an IDS flag. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Event ID|Specify the ID or UUID of the event from which you want to download files||String||
|Download Folder Path|Specify the absolute path to the folder, which should store files. If nothing is specified, action will create an attachment instead. Note: JSON result is only available, when you provide proper value for this parameter.||String||
|Overwrite|If enabled, action will overwrite existing files.||Boolean|false|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers for which you want to unset an IDS flag. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to seach for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“.||String||
...
|Type|Specify a comma-separated list of attribute types. If specified, action will only unset IDS flag for attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and unset IDS flag for all attributes that match our criteria.||List|Provided Event|
|Attribute UUID|Specify a comma-separated list of attribute UUIDs for which you want to unset an IDS flag. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
...
|Return Attributes Info|If enabled, action will create a case wall table for all of the attributes that are a part of the event.||Boolean|True|
...
|Max Objects to Return|Specify how many objects to return.||String|50|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers to which you want to add tags. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to search for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“ or Object UUID is provided.||String||
|Tag Name|Specify a comma-separated list of tags that you want to add to attributes.|True|String||
|Category|Specify a comma-separated list of categories. If specified, action will only add tags to attributes that have matching category. If nothing is specified, action will ignore categories in attributes. Possible values: External Analysis, Payload Delivery, Artifacts Dropped, Payload Installation.||String||
|Type|Specify a comma-separated list of attribute types. If specified, action will only add tags to attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Object UUID|Specify the uuid of the object that contains the desired attribute.||String||
|Attribute UUID|Specify a comma-separated list of attribute UUIDs to which you want to add new tags. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and add sighting for all attributes that match our criteria.||List|Provided Event|
...
|Distribution|Specify the distribution of the event. Possible values: 0 - Organisation, 1 - Community, 2 - Connected, 3 - All. You can either provide a number or a string.||String|Community|
|Threat Level|Specify the threat level of the event. Possible values: 1 - High, 2 - Medium, 3 - Low, 4 - Undefined. You can either provide a number or a string.||String|High|
|Analysis|Specify the analysis of the event. Possible values: 0 - Initial, 1 - Ongoing, 2 - Completed. You can either provide a number or a string.||String|Initial|
|Publish|If enabled, action will publish the event to the community.||Boolean|false|
|Comment|Specify additional comments related to the event.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Attribute Name|Specify a comma-separated list of attribute identifiers from which you want to remove tags. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Event ID|Specify the ID or UUID of the event, where to search for attributes. This parameter is required, if “Attribute Search“ is set to “Provided Event“ or Object UUID is provided.||String||
...
|Type|Specify a comma-separated list of attribute types. If specified, action will only remove tags from attributes that have matching attribute type. If nothing is specified, action will ignore types in attributes. Example values: md5, sha1, ip-src, ip-dst||String||
|Object UUID|Specify the UUID of the object that contains the desired attribute.||String||
|Attribute UUID|Specify a comma-separated list of attribute UUIDs from which you want to remove new tags. Note: If both “Attribute Name“ and “Attribute UUID“ are specified, action will work with “Attribute UUID“ values.||String||
|Attribute Search|Specify, where action should search for attributes. If “Provided Event“ is selected, action will only search for attributes or attribute UUIDs in event with ID/UUID provided in “Event ID“ parameter. If “All Events“, action will search for attributes among all events and remove tags from all attributes that match our criteria.||List|Provided Event|
...
|Comment|Specify the comment, which you want to add to the event.||String||
|Detection Ratio|Specify the detection ration, which you want to add to the event.||String||
|Community Score|Specify the community score, which you want to add to the event.||String||
|First Submission|Specify first submission of the event. Format: 2020-12-22T13:07:32Z||String||
|Last Submission|Specify last submission of the event. Format: 2020-12-22T13:07:32Z||String||
...
|Src-port|Specify the source port, which you want to add to the event.||String||
|Domain|Specify the domain, which you want to add to the event.||String||
|HOSTNAME|Specify the hostname, which you want to add to the event.||String||
|IP-Src|Specify the source IP, which you want to add to the event.||String||
|IP-Dst|Specify the destination IP, which you want to add to the event.||String||
|Use Entities|If enabled, action will use entities in order to create objects. Supported entities: IP Address. “Use Entities“ has priority over other parameters.||Boolean|false|
|IP Type|Specify what attribute type should be used with IP entities.||List|Source IP|
...
|Category|Specify the category for attributes. Possible values: Targeting data, Payload delivery, Artifacts dropped, Payload installation, Persistence mechanism, Network activity, Attribution, External analysis, Social network.||String||
|Distribution|Specify the distribution of the attribute. Possible values: 0 - Organisation, 1 - Community, 2 - Connected, 3 - All, 5 - Inherit. You can either provide a number or a string.||String|Community|
|For Intrusion Detection System|If enabled, attribute will be labeled as eligible to create an IDS signature out of it.||Boolean|false|
|Comment|Specify comment related to attribute.||String||
|Fallback IP Type|Specify what should be the fallback attribute type for the IP address entity.||List|Source Address|
|Fallback Email Type|Specify what should be the fallback attribute type for the email address entity.||List|Source Email Address|
|Extract Domain|If enabled, action will extract domain out of URL entity.||Boolean|true|
...
|Category|Specify the category for the uploaded file. Possible values: External Analysis, Payload Delivery, Artifacts Dropped, Payload Installation.||String||
|Distribution|Specify the distribution for the uploaded file. Possible values: 0 - Organisation, 1 - Community, 2 - Connected, 3 - All. You can either provide a number or a string.||String|Community|
|For Intrusion Detection System|If enabled, uploaded file will be used for intrusion detection systems.||Boolean|False|
|Comment|Specify additional comments related to the uploaded file.||String||
...
|Attribute Type Filter|Filter attributes by their type, comma separated. If provided, only attributes with whitelisted type will be processed.||String||
|Category Filter|Filter attributes by their category, comma separated. If provided, only attributes with whitelisted category will be processed.||String||
|Galaxy Filter|Filter attributes by their parent event's galaxy, comma separated. If provided, only attributes that belong to an event with a whitelisted galaxy will be processed.||String||
|Verify SSL|If enabled, verify the SSL certificate for the connection to the CheckPoint Cloud Guard server is valid.||Boolean|false|
...
|CA Certificate File - parsed into Base64 String|CA Certificate File - parsed into Base64 String||String||
