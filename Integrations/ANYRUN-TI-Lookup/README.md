...
|Verify SSL|Enable to validate SSL certificates for API connections.||Boolean|true|
|Enable proxy|Enable/disable proxy using.||Boolean|false|
|Proxy host|Specify proxy host.||String||
|Proxy port|Specify proxy port.||String||
...
|Query|Raw query with necessary filters. Supports condition concatenation with AND, OR, NOT and Parentheses ().||String||
