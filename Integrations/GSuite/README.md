<p align="center"><img src="./Resources/GSuite.svg" 
     alt="GSuite" width="200"/></p>
...
|----|-----------|-----------|----|------------|
|Verify SSL|If enabled, the server-side certificate used for connection is validated.||Boolean||
|Service Account JSON|For the service account based authentication, specify the Service Account JSON. Parameter accepts two ways to provide the service account: First (recommended), the full JSON content of the service account can be provided. The second way (legacy, will be deprecated) is to provide the path to the stored service account JSON file on the CSOAR server.||Password|*****|
|Workload Identity Email|For the service account based authentication, specify the Workload Identity email.||String||
|Delegated Email|Specify the email that integration should use.||String||
...
|Check Roles|Specify a comma-separated list of roles that you want to check in relation to the group.||String||
|Check Privileges|Specify a comma-separated list of permission that you want to check in relation to the group. Note: "Expand Privileges" needs to be enabled for this parameter to work. If there are values inside the "Check Roles" parameter, action will check the privileges only among those roles.||String||
|Expand Privileges|If enabled, action will return information about all of the unique privileges related to the group.||Boolean||
...
|Organization Unit Path|The path of the organizational unit that needs to be returned.||String|/|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|User Email Addresses|A comma-separated list of users that you want to remove from the group. Note: values from this parameter will be executed alongside User Entities.||String||
|Check Roles|Specify a comma-separated list of roles that you want to check in relation to the user.||String||
|Check Privileges|Specify a comma-separated list of permission that you want to check in relation to the user. Note: "Expand Privileges" needs to be enabled for this parameter to work. If there are values inside the "Check Roles" parameter, action will check the privileges only among those roles.||String||
|Include Inherited Roles|If enabled, action will additionally return user roles that were inherited from groups.||Boolean||
|Expand Privileges|If enabled, action will return information about all of the unique privileges related to the user.||Boolean||
...
|Name|Display name of the OU.||String||
|Description|Description of the OU.||String||
...
|Include Derived Membership|Whether to list indirect memberships.||Boolean|true|
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Customer ID|The unique ID for the customer's G Suite account, 'my_customer' alias can also be used to represent your account's customerId.||String||
|Domain|Specify a domain to search for users in.||String||
|Manager Email|The email address of a user's direct manager||String||
|Return only Admin Accounts?|Specify whether to return only admin accounts.||Boolean|false|
|Return only Delegated Admin Accounts?|Specify whether to return only delegated admin accounts.||Boolean|false|
|Return only Suspended Users?|Specify whether to return only suspended accounts.||Boolean|false|
|Org Unit Path|The full path of an org unit to retrieve users from. This matches all org unit chains under the target.||String||
|Department|The department within the organization to retrieve users from.||String||
...
|Return only users without 2fa?|If enabled, action will only return users that don't have 2fa enabled.||Boolean|false|
|Email Addresses|Specify a comma-separated list of email addresses that need to be searched for. Note: if 'Email Addresses' parameter is used, then 'Record Limit' parameter is ignored.||String||
...
|Name|Display name of the new group.||String||
|Description|Description of the new group.||String||
...
|Gender|The gender of the user. Valid values: female, male, other, unknown.||String||
|Department|The name of the department of the user.||String||
|Organization|The name of the organization of the user.||String||
|Change Password At Next Login|Whether to force the user to change his password on next login.||Boolean|false|
...
|----|-----------|-----------|----|------------|
|Customer ID|The unique ID for the customer's G Suite account, 'my_customer' alias can also be used to represent your account's customerId.|True|String||
|Name|Display name of the new OU.||String||
|Description|Description of the new OU.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|User Email Addresses|A comma-separated list of users to sign out. The action runs the values from this parameter with User entities.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|User Email Addresses|A comma-separated list of additional user email addresses to process.The action processes all users in this list in addition to a User entity (if one is provided).||String||
...
|Event Type Filter|A comma-separated list of event types to retrieve.||String||
|Time Frame|The time frame for the activity search. If Custom is selected, the Start Time parameter is required.The possible values are as follows:     • Last Hour    • Last 6 Hours    • Last 24 Hours    • Last Week    • Last Month    • Custom||List|Last Hour|
|Start Time|The start of the time range for the activity search.This parameter is required if Custom is selected for the Time Frame parameter.Configure the value in ISO 8601 format.||String||
|End Time|The end of the time range for the activity search.This parameter is optional when Custom is selected for the Time Frame parameter and defaults to the current time if not provided.Configure the value in ISO 8601 format.||String||
|Max Events To Return|The maximum number of events to return per user. The default value is 200.The maximum value is 1000.The action processes a maximum of 1000 events per user, per application.||String|200|
...
|Family Name|The user's last name||String||
...
|Organization|The name of the organization of the user.||String||
|Change Password At Next Login|Whether to force the user to change his password on next login.||Boolean|false|
|User Status|Specify if user status should be updated to blocked or unblocked. By default action is no changing the user status.||List|Not Changed|
