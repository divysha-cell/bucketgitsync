...
|Custom Query Fields|None||String||
...
|Perform Nested Search|Specify whether the action should fetch additional details regarding groups found in the main group.||Boolean|false|
...
|Limit|Specify the maximum number of listings to fetch from Active Directory.||String||
...
|Name|Description|IsMandatory|Type|DefaultValue|
|----|-----------|-----------|----|------------|
|Mark entities as internal|Specify whether successfully enriched entities should be automatically marked as “Internal Entity”||Boolean||
|Specific Attribute Names To Enrich With|Provide a comma separated list of attribute names to enrich the entities with. If nothing is provided - action will enrich with all available attributes. If an attribute contains a few values - it will be enriched with all of the available values. Parameter is case sensitive.||String||
|Should Case Wall Table be filtered by the specified Attributes?|If checked, the Case Wall Table for this action will only present the specified attributes, found in the “Specific Attribute Names To Enrich With” parameter.||Boolean|false|
|Should JSON result be filtered by the specified Attributes?|If checked, the JSON result for this action will only return the specified attributes, found in the “Specific Attribute Names To Enrich With” parameter.||Boolean|false|
