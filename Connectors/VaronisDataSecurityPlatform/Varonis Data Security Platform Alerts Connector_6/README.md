# Varonis Data Security Platform Alerts Connector_6
...
|Proxy Server Address|Proxy server to use for connection to the mail server.|False|0|
|Proxy Username|Proxy server username|False|0|
...
If the environment field isn't found, the environment is "".|False|0|
...
|Script Timeout (Seconds)|Timeout limit for the python process running the connector.|True|300|
|API Root|Specify the API URL for the target Varonis Data Security Platform instance.|True|https://{ip address}:{port}|
|Username|Specify the username to connect with.|True|0|
