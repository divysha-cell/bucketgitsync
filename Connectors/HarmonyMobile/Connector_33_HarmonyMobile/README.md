...
|API Root|API root of the Harmony Mobile instance.|True|https://cloudinfra-gw.portal.checkpoint.com|
|Client ID|Client ID of the Harmony Mobile account.|True|dummy_valid_string|
|Client Secret|Client Secret of the Harmony Mobile account. |True|*****|
|Verify SSL|If enabled, verify the SSL certificate for the connection to the Harmony Mobile server is valid.|False|true|
|Lowest Risk To Fetch|Lowest risk that needs to be used to fetch alerts. Possible values: Informational, Low, Medium, High. If nothing is specified, the connector will ingest alerts with all risk levels.|False|dummy_valid_string|
